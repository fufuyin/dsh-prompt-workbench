...
const MODE_GUIDE = {
  refine:
    'Clarify and expand. Turn the vague wish into concrete, checkable requirements: state the real goal, the expected outcome, and the concrete deliverables or steps. Supply the obviously implied context, but never invent facts the author did not imply.',
  constraints:
    'Add constraints. Make the implicit boundaries explicit: in scope and out of scope, platform or technical limits, style and compatibility rules, non-goals, and the acceptance criteria the result must satisfy.',
  structure:
    'Restructure. Reorganize the whole prompt into a clean, scannable hierarchy (for example: 目标 / 背景 / 输入 / 具体要求 / 输出格式 / 验收标准). Keep every original detail — change the shape, not the content.',
  concise:
    'Compress. Remove filler, hedging, and repetition, merge overlapping statements, and keep every distinct fact and requirement. The result must be clearly shorter without losing information.',
} as const

/** The rewrite modes in presentation order. */
export const MODE_KEYS = ['refine', 'constraints', 'structure', 'concise'] as const
...
export function normalizeMode(value: unknown): RewriteMode {
  return typeof value === 'string' && (MODE_KEYS as readonly string[]).includes(value)
...
export function outputLanguage(language: OutputLanguage, source: string): 'zh' | 'en' {
