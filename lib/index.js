...
import { inspectOrigin, queryParam, readJsonBody, sendJson } from "./http.js";
...
export function apply(ctx) {
     ...
     */
    const route = (kind, path, handler) => webServer.register({
        kind,
        path,
        handler: async (request, response) => {
            ...
            while (recent.length > REQUEST_RING_SIZE)
                recent.shift();
        },
    });
    /**
    ...
    const diag = route('exact', `${API}/diag`, (_request, response) => {
        ...
        let clientModules;
        try {
            clientModules = host.get('clientModules');
        }
        catch (error) {
