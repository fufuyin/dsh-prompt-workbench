...
const DIMENSION_ORDER = ['goal', 'context', 'constraints', 'output', 'acceptance', 'examples'];
/**
...
function collectVagueness(text) {
    ...
    for (const phrase of VAGUE_EN) {
        ...
        if (match !== null)
            found.push({ phrase, at: match.index });
    }
 ...
 */
export function outlineFor(mode) {
    if (mode === 'concise')
        return [];
    if (mode === 'constraints') {
        return [
 ...
 */
export function analyzePrompt(text, mode) {
 ...
 */
export function describeGapsForModel(analysis) {
    const lines = [];
